...
/*                sorting               */    
NODE sortedmerge(NODE a,NODE b,int i){
	NODE result=NULL;
	if(a==NULL)
		return b;
	else if(b==NULL)
	...
	if(i==1){
		if(a->bt <= b->bt){
			result=a;
			result->link=sortedmerge(a->link,b,i);
		}
		else{
			result=b;
			result->link=sortedmerge(a,b->link,i);
		}
	}
	if(i==2){
		if(a->prt <= b->prt){
...
void mergesort(NODE temp,int i){
	NODE ptr=temp,a,b;
	...
	mergesort(a,i);
	mergesort(b,i);
	temp=sortedmerge(a,b,i);
}
void dis(NODE temp){
	NODE ptr=temp;
	printf("\n");
	while(ptr!=NULL){
		printf("%.1f %d",ptr->bt,ptr->prt);
		ptr=ptr->link;
	}
}
...
void priority(int process,float *fr){
	float ttat=0,twt=0,trd=0,trt=0,tbt=0;
	copy();
	mergesort(START,2);
	dis(START);
	...
	do
	{
		...
		switch(ch){
			case 1:printf("\nFCFS scheduling\n");
				...
				break;
			case 2:printf("\npriority scheduling\n");
				priority(process,&first_response);
				break;
			case 3:printf("\nSJF scheduling\n");
